...
public class Attribute<T> extends Pair<T, T> {
    ...
    public static void Decode(String encoded, Header h) {
        if(!encoded.contains(Attribute.KeyValuePairSeparator)) {
            h.raw = null;
            return;
        }
        encoded = encoded.replaceFirst(h.toString()+Attribute.KeyValuePairSeparator, "").trim();
        for (String keyValuePairs : encoded.split(KeyValuePairSeparator)) {
            try {
